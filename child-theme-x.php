...
/**
 * Child Theme X
 * @author    Essential Themes <l@essentialthemes.co>
 * @link      https://essentialthemes.co
 * @copyright 2020
 * @package   Child Theme X
 *
 * @wordpress-plugin
 * Plugin Name: Child Theme X
 * Plugin URI:
 * Author: Essential Themes
 * Author URI:
 * Version: 1.0
 * Description: Create Child Themes and change the theme quickly.
 * Text Domain: child-theme-x
 * Domain Path: /languages
 * License: GPL-2.0+
 * License URI: http://www.gnu.org/licenses/gpl-2.0.txt
 */
final class child_theme_x{

	private static
	/**
	 * Current theme information
	 * @since	1.0		2020-04-20		Release
	 * @access  private
	 * @var     mixed
	 */
	$current_theme = null,
...
     * with their respective parameters.
    ...
    public static function create_admin_menu(){
		...
		$menu_label_ex = '<span class="ab-icon dashicons-admin-appearance"></span><span class="ab-label">' .
						sprintf( __( 'Theme: %s', 'child-theme-x' ) , '<strong>' . $menu_label . '</strong>' ) . '</span>';
...
     * Through Cookie and only works for the administrator role for testing.
	 ...
	 * @return void
	 */
	private function sanitize_data_ajax(){
	 ...
	 */
	public function create_style_css() {
		// Call sanitize data
		...
		$txt .= "*/\n\n";
		$txt .= "/* ";
		$txt .=  esc_html__('Write here your own personal stylesheet', 'child-theme-x');
		$txt .= " */\n";
		...
		//chmod($style_css, 0775);
		//if( ! chmod( $style_css, 0775) ){ echo "!!";exit; }
	 ...
	 * @return	int		1 = It worked, 0 = An error occurred
	 */
	public function create_functions_php() {
		$fn_slug = str_replace( "-", "_", self::$request['text_domain'] );
		// functions.php content
		$txt = "";
		$txt .= "<?php\n";
		$txt .= "/*";
		$txt .= esc_html__('This file is part of ', 'child-theme-x' );
		$txt .= self::$request['text_domain'] . ", " . self::$request['parent'] . " child theme.\n\n";
		$txt .= esc_html__('All functions of this file will be loaded before of parent theme functions.', 'child-theme-x') . "\n";
		$txt .= esc_html__('Learn more at ', 'child-theme-x') . 'https://codex.wordpress.org/Child_Themes.' . "\n\n";
		$txt .= esc_html__('Note: this function loads the parent stylesheet before, then child theme stylesheet', 'child-theme-x') . "\n";
		$txt .= esc_html__('(leave it in place unless you know what you are doing.)', 'child-theme-x') . "\n*/\n";
		$txt .= "
	...
	wp_enqueue_style( 'childe2-style');
		...
		$txt .= "/*";
		$txt .= esc_html__('Write here your own functions', 'child-theme-x') . " */\n";
	 ...
	 * @return	int		1 = It worked, 0 = An error occurred
	 */
	public function create_screenshot_png() {
	 ...
	 * @return	bool
	 */
	private function is_localhost($whitelist = ['127.0.0.1', '::1', 'localhost']) {
    ...
    public static function popup(){
        ...
        ?>
<div class="tsc-popup" host="<?php echo urlencode(self::get_host()); ?>" data-name-admin="<?php echo $first_name . ' ' . $last_name; ?>">
...
jQuery(document).ready(function( $ ){
	...
	$('#theme-sc-button-generate').on('click', function(){
		...
		/* window.wp.ajax.post( 'create-child-action', data )
		.done( function( response ) {
			//window.location.reload(true);
			if( response.success ){
				setTimeout(() => {
					window.location.href= decodeURIComponent( $('#theme-sc-url-page-themes').val() ) + '?theme=' + response.sheetstyle;
				}, 2500);
			}else{
				alert( "Error, please check console" );
			}
...
/* #wp-admin-bar-theme-sc-themeswitch .ab-submenu > li:not(.theme-sc-header) .tsc-current-view{
	font-size: 11px;
    border: 1px solid;
...
.tsc-popup-overlow.tsc-active{
	 ...
	 * @return 	string|html
	 */
	public function wp_footer_script_pointer() {
	 ...
	 * @return 	void
	 */
	public function ajax_point_viwed(){
...
*/
// ─── Hook Backend ────────
add_action( 'admin_footer', ['child_theme_x','footer'], 10 );
add_action( 'admin_footer', ['child_theme_x','popup'], 11 );

add_action( 'wp_ajax_create-child-action', ['child_theme_x','ajax_create_child_theme'] );
add_action( 'wp_ajax_current-user-view-point', ['child_theme_x','ajax_point_viwed'] );

add_action( 'admin_head', ['child_theme_x','header'], 10 );
add_action( 'admin_bar_menu',  [ 'child_theme_x', 'create_admin_menu' ], 100  );

add_action( 'admin_enqueue_scripts', ['child_theme_x','admin_enqueue_scripts'], 10 );


// ─── Hook Frontend ────────
add_action( 'wp_head', ['child_theme_x','header'], 10 );
add_action( 'wp_footer', ['child_theme_x','footer'], 10 );
add_action( 'wp_footer', ['child_theme_x','popup'], 11 );


// ─── Hook | Both of them ────────
add_action( 'switch_theme', ['child_theme_x','after_theme_switch']);
add_filter( 'template',  ['child_theme_x','themesc_swicth_template'], 10 );
add_filter( 'stylesheet', ['child_theme_x','themesc_swicth_stylesheet'], 10 );